...
const LAYOUT_KEY = "brasix:v1:product-editor-v3-layout";
const THEME_KEY = "brasix:v1:product-editor-v3-theme";
...
const OPERATIONAL_AUTOSAVE_DELAY_MS = 600;
const OPERATIONAL_AUTOFILL_POLL_INTERVAL_MS = 1600;
const SEASONALITY_MONTHS = [
  { field: "seasonality_index_jan", label: "J" },
  { field: "seasonality_index_feb", label: "F" },
  { field: "seasonality_index_mar", label: "M" },
  { field: "seasonality_index_apr", label: "A" },
  { field: "seasonality_index_may", label: "M" },
  { field: "seasonality_index_jun", label: "J" },
  { field: "seasonality_index_jul", label: "J" },
  { field: "seasonality_index_aug", label: "A" },
  { field: "seasonality_index_sep", label: "S" },
  { field: "seasonality_index_oct", label: "O" },
  { field: "seasonality_index_nov", label: "N" },
  { field: "seasonality_index_dec", label: "D" },
];
function uniqueEmojiList(items) {
  const seen = new Set();
  const emojis = [];
  (items || []).forEach((item) => {
    const emoji = String(item || "");
...
const PRODUCT_EMOJI_GROUPS = [
  {
    ...
    emojis: uniqueEmojiList(["🌱", "🌿", "🍃", "🌾", "🌽", "🌻", "🪴", "🌳", "🌴", "🎋", "🪵", "🪨", "⛰️", "🌋", "🔥"]),
  },
  {
    label: "Frutas, legumes e grãos",
    emojis: uniqueEmojiList(["🍎", "🍐", "🍊", "🍋", "🍌", "🍉", "🍇", "🍓", "🫐", "🍈", "🍒", "🍑", "🥭", "🍍", "🥥", "🥝", "🍅", "🫒", "🥑", "🍆", "🥔", "🥕", "🌶️", "🫑", "🥒", "🥬", "🥦", "🧄", "🧅", "🍄", "🫘", "🌰", "🫚"]),
  },
  {
    label: "Animais e pecuária",
    emojis: uniqueEmojiList(["🐄", "🐂", "🐃", "🐖", "🐏", "🐑", "🐐", "🐓", "🐔", "🦃", "🦆", "🐇", "🐟", "🐠", "🦐", "🦞", "🦀", "🦑", "🐙", "🥛", "🥚"]),
  },
  {
  ...
  },
  {
    ...
    emojis: uniqueEmojiList(["📦", "🛒", "🎁", "🧴", "🧼", "🪥", "🧻", "🧷", "🪡", "🧵", "🧶", "👕", "👖", "🧥", "🧦", "👗", "👚", "👔", "👜", "🎒", "👞", "👟", "🥿", "👠", "🥾", "🪑", "🛏️", "🚪", "🪟", "🛋️", "🪞", "🧸", "⚽", "🏀", "🏐", "🎾", "🏈", "⚾", "🎱"]),
  },
  {
    label: "Indústria e materiais",
    emojis: uniqueEmojiList(["🏗️", "🧱", "⚙️", "🔩", "⛓️", "🧪", "🛢️", "⛏️", "🪙", "💎", "🧲", "🪚", "🪛", "🔧", "🪜", "🧰", "🔋", "🔌", "💡"]),
  },
  {
    label: "Tecnologia e eletrônicos",
    emojis: uniqueEmojiList(["📱", "☎️", "📞", "⌚", "⏱️", "💻", "🖥️", "⌨️", "🖨️", "🖱️", "🖲️", "💿", "📀", "📷", "📸", "📹", "🎥", "📺", "📻", "🎧", "🎙️", "🛰️", "📡"]),
  },
  {
    label: "Transporte e energia",
    emojis: uniqueEmojiList(["🚚", "🚛", "🚜", "🚐", "🚗", "🏍️", "🚲", "🛺", "🚢", "⛽", "⚡"]),
  },
];
const PRODUCT_EMOJI_OPTIONS = uniqueEmojiList(PRODUCT_EMOJI_GROUPS.flatMap((group) => group.emojis));
...
const state = {
  ...
  createDraft: null,
  operationalByProductId: {},
  operationalDraftsByProductId: {},
  operationalAutosaveTimersByProductId: {},
  operationalSaveChain: Promise.resolve(),
  operationalSavingByProductId: {},
  operationalAutofillStatusByProductId: {},
  operationalAutofillPollTimer: null,
  operationalAutofillPollingProductId: "",
};
...
function numericInputValue(value, digits = null) {
...
function formatCurrencyValue(value) {
  ...
  const digits = Math.abs(numericValue) >= 100 ? 0 : 2;
  return `R$ ${formatNumber(numericValue, digits)}`;
}

function resolveProductUnit(product = currentProduct()) {
  return String(product?.unit || "").trim() || "un";
}

function normalizeProductOperationalRecord(record, product) {
  const nextRecord = {
    product_id: String(record?.product_id || product?.id || "").trim(),
    unit: String(record?.unit || resolveProductUnit(product)).trim() || resolveProductUnit(product),
    weight_per_unit_kg: toOptionalNumber(record?.weight_per_unit_kg),
    volume_per_unit_m3: toOptionalNumber(record?.volume_per_unit_m3),
    price_reference_brl_per_unit: toOptionalNumber(record?.price_reference_brl_per_unit),
    price_min_brl_per_unit: toOptionalNumber(record?.price_min_brl_per_unit),
    price_max_brl_per_unit: toOptionalNumber(record?.price_max_brl_per_unit),
    confidence: String(record?.confidence || "").trim(),
    research_basis: String(record?.research_basis || "").trim(),
    source_urls: normalizeSourceUrls(record?.source_urls),
    notes: String(record?.notes || "").trim(),
  };

  const monthValues = SEASONALITY_MONTHS.map(({ field }) => {
    const value = toOptionalNumber(record?.[field]);
    return value == null ? 1.0 : value;
  });
  const inferredSeasonal = monthValues.some((value) => Math.abs(value - 1) > 0.0001);
  nextRecord.is_seasonal = typeof record?.is_seasonal === "boolean" ? record.is_seasonal : inferredSeasonal;

  SEASONALITY_MONTHS.forEach(({ field }, index) => {
      nextRecord[field] = monthValues[index];
  });
...
function buildOperationalIndex(items) {
  ...
  (items || []).forEach((item) => {
    ...
    byProductId[productId] = normalizeProductOperationalRecord(item, state.productsById[productId]);
  });
  return byProductId;
}

function currentOperationalRecord(productId = state.selectedProductId) {
  return state.operationalByProductId[productId] || null;
}

function currentOperationalDraft(productId = state.selectedProductId) {
  const product = state.productsById[productId] || null;
  if (!product) {
    return null;
  }
  if (!state.operationalDraftsByProductId[productId]) {
    state.operationalDraftsByProductId[productId] = deepClone(
      currentOperationalRecord(productId) || normalizeProductOperationalRecord({ product_id: productId }, product),
    );
  }
  return state.operationalDraftsByProductId[productId];
}

function operationalAutofillStatus(productId = state.selectedProductId) {
  return state.operationalAutofillStatusByProductId[productId] || {
...
function seasonalityValues(record) {
  return SEASONALITY_MONTHS.map(({ field }) => {
    const value = toOptionalNumber(record?.[field]);
    return value == null ? 1.0 : value;
  });
}

function buildOperationalSavePayload(productId = state.selectedProductId, sourceRecord = null) {
  const product = state.productsById[productId] || null;
  if (!product) {
    return null;
  }
  const source = normalizeProductOperationalRecord(sourceRecord || currentOperationalDraft(productId) || { product_id: productId }, product);
  return {
    product_id: productId,
    unit: resolveProductUnit(product),
    weight_per_unit_kg: source.weight_per_unit_kg,
    volume_per_unit_m3: source.volume_per_unit_m3,
    price_reference_brl_per_unit: source.price_reference_brl_per_unit,
    price_min_brl_per_unit: source.price_min_brl_per_unit,
    price_max_brl_per_unit: source.price_max_brl_per_unit,
    is_seasonal: Boolean(source.is_seasonal),
    seasonality_index_jan: toOptionalNumber(source.seasonality_index_jan) ?? 1.0,
    seasonality_index_feb: toOptionalNumber(source.seasonality_index_feb) ?? 1.0,
    seasonality_index_mar: toOptionalNumber(source.seasonality_index_mar) ?? 1.0,
    seasonality_index_apr: toOptionalNumber(source.seasonality_index_apr) ?? 1.0,
    seasonality_index_may: toOptionalNumber(source.seasonality_index_may) ?? 1.0,
    seasonality_index_jun: toOptionalNumber(source.seasonality_index_jun) ?? 1.0,
    seasonality_index_jul: toOptionalNumber(source.seasonality_index_jul) ?? 1.0,
    seasonality_index_aug: toOptionalNumber(source.seasonality_index_aug) ?? 1.0,
    seasonality_index_sep: toOptionalNumber(source.seasonality_index_sep) ?? 1.0,
    seasonality_index_oct: toOptionalNumber(source.seasonality_index_oct) ?? 1.0,
    seasonality_index_nov: toOptionalNumber(source.seasonality_index_nov) ?? 1.0,
    seasonality_index_dec: toOptionalNumber(source.seasonality_index_dec) ?? 1.0,
    confidence: String(source.confidence || "").trim() || null,
    research_basis: String(source.research_basis || "").trim() || null,
    source_urls: normalizeSourceUrls(source.source_urls),
    notes: String(source.notes || "").trim(),
  };
}

function serializeOperationalPayload(payload) {
  return JSON.stringify(payload || {});
}

function isOperationalDirty(productId = state.selectedProductId) {
  const draftPayload = buildOperationalSavePayload(productId);
  const savedPayload = buildOperationalSavePayload(productId, currentOperationalRecord(productId));
  if (!draftPayload || !savedPayload) {
    return false;
  }
  return serializeOperationalPayload(draftPayload) !== serializeOperationalPayload(savedPayload);
}

function rememberSavedOperationalRecord(record, { overwriteDraft = false } = {}) {
  const productId = String(record?.product_id || "").trim();
  const product = state.productsById[productId] || null;
  if (!productId || !product) {
    return;
  }
  const normalized = normalizeProductOperationalRecord(record, product);
  state.operationalByProductId[productId] = normalized;
  if (overwriteDraft || !isOperationalDirty(productId)) {
    state.operationalDraftsByProductId[productId] = deepClone(normalized);
  }
}

function clearOperationalAutosave(productId = state.selectedProductId) {
  const timer = state.operationalAutosaveTimersByProductId[productId];
  if (!timer) {
    return;
  }
  window.clearTimeout(timer);
  delete state.operationalAutosaveTimersByProductId[productId];
}
...
function loadBootstrap() {
  return fetch("/api/editor/products_v3/bootstrap").then(async (response) => {
    if (!response.ok) {
      throw new Error(`Falha ao carregar bootstrap do editor de produtos v3 (${response.status}).`);
    }
...
function loadFieldDocument(mapId, productId, layer) {
  ...
  return fetch(`/api/editor/products_v3/field?${query.toString()}`).then(async (response) => {
    if (!response.ok) {
...
function saveFieldDocument(payload) {
  return fetch("/api/editor/products_v3/field", {
    method: "PUT",
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (Date.now() - startedAt >= timeoutMs) {
        window.clearInterval(timer);
        reject(new Error("Leaflet nao carregou a tempo para o editor de produtos v3."));
      }
    }, 50);
  });
}
...
function applyTheme(themeId, { persist = true } = {}) {
  ...
  applyCssVariables(state.bootstrap?.product_editor_v3?.layout_desktop?.css_variables || {});
...
function initializeState(payload) {
  const previousDrafts = { ...state.operationalDraftsByProductId };
  const previousStatuses = { ...state.operationalAutofillStatusByProductId };
  state.bootstrap = payload;
  state.screen = payload.product_editor_v3?.screen || {};
  state.shortcuts = payload.product_editor_v3?.shortcuts || { items: [] };
  state.themesDocument = payload.product_editor_v3?.themes || {};
  ...
  state.operationalByProductId = buildOperationalIndex(payload.product_operational_catalog?.items || []);
  state.operationalDraftsByProductId = {};
  state.products.forEach((product) => {
    state.operationalDraftsByProductId[product.id] = previousDrafts[product.id]
      ? normalizeProductOperationalRecord(previousDrafts[product.id], product)
      : deepClone(
        state.operationalByProductId[product.id]
          || normalizeProductOperationalRecord({ product_id: product.id }, product),
      );
  });
  state.operationalAutofillStatusByProductId = Object.fromEntries(
    Object.entries(previousStatuses).filter(([productId]) => Boolean(state.productsById[productId])),
  );
...
function computeLayerTotal(productId, layer) {
  return state.cities.reduce((total, city) => total + computeLayerInfo(productId, city, layer).finalValue, 0);
}

function buildSeasonalityScale(values) {
  const numericValues = (values || []).map((value) => Number(value || 0));
  const minValue = Math.min(...numericValues, 1);
  const maxValue = Math.max(...numericValues, 1);
  const paddedDistance = Math.max(Math.abs(maxValue - 1), Math.abs(1 - minValue), 0.01) * 1.1;
  return {
    min: 1 - paddedDistance,
    max: 1 + paddedDistance,
...
function inputChoiceMarkup(field, label, value, { type = "text", step = null, min = null, placeholder = "", disabled = false } = {}) {
  return `
    <label class="field truck-operations-choice">
      <span>${escapeHtml(label)}</span>
      <input
        class="editor-input"
...
function textareaChoiceMarkup(field, label, value, { rows = 4, compact = false, placeholder = "" } = {}) {
  return `
    <label class="field truck-operations-choice">
      <span>${escapeHtml(label)}</span>
      <textarea
        class="editor-input product-editor-operational-textarea${compact ? " is-compact" : ""}"
...
function sectionBoxMarkup(title, innerMarkup, description = "") {
  const headMarkup = title || description
    ? `
      <div class="product-editor-section-head">
        ${title ? `<p class="truck-operations-stack-label">${escapeHtml(title)}</p>` : ""}
        ${description ? `<span class="product-editor-section-copy">${escapeHtml(description)}</span>` : ""}
      </div>
    `
    : "";
  return `
...
function stackBoxMarkup(innerMarkup) {
  return `
    <section class="truck-operations-section-box">
      <div class="truck-operations-choice-stack">
        ${innerMarkup}
...
function renderProductDetail(rows = null) {
  ...
  target.innerHTML = stackBoxMarkup(
    [
      staticChoiceMarkup("Produto", `${product.emoji || "\u{1F4E6}"} ${product.name}`.trim()),
      staticChoiceMarkup("Familia", familyLabel(product.family_id)),
      staticChoiceMarkup("Tipo logistico", logisticsLabel(product.logistics_type_id)),
      staticChoiceMarkup("Unidade", product.unit || "-"),
      staticChoiceMarkup("Sessoes manuais", `${(currentDoc?.strokes || []).length}`),
      staticChoiceMarkup("Insumos", inputNames.length ? inputNames.join(", ") : "-"),
      staticChoiceMarkup("Derivacoes", outputNames.length ? outputNames.join(", ") : "-"),
    ].join(""),
  );
}
...
function renderCityDetail() {
  ...
  const note = labels().pending_city_edit_label || "";
  target.innerHTML = `
    ${sectionBoxMarkup(
      "Cidade",
      [
        staticChoiceMarkup("Cidade selecionada", city.label || city.name || "-"),
        staticChoiceMarkup("Produto", product.name || "-"),
        staticChoiceMarkup(labels().city_state_label || "UF", city.state_code || "-"),
        staticChoiceMarkup(labels().city_population_label || "Populacao", `${formatNumber(city.population_thousands || 0, 0)} mil`),
        staticChoiceMarkup("Camada atual", layerLabel(state.selectedLayer)),
        staticChoiceMarkup("Oferta final", formatUnitValue(supplyInfo.finalValue, product.unit)),
        staticChoiceMarkup("Demanda final", formatUnitValue(demandInfo.finalValue, product.unit)),
      ].join(""),
    )}
    ${note ? `<p class="product-editor-stack-note">${escapeHtml(note)}</p>` : ""}
  `;
}

function renderInformationPanel() {
  const target = document.getElementById("product-editor-v3-information-panel");
  const product = currentProduct();
  const draft = currentOperationalDraft();
  if (!target || !product || !draft) {
    return;
  }

  const savePending = Boolean(state.operationalAutosaveTimersByProductId[product.id]);
  const saveInFlight = Boolean(state.operationalSavingByProductId[product.id]);
  const autofill = operationalAutofillStatus(product.id);
  const unit = resolveProductUnit(product);
  const storedMonthValues = seasonalityValues(draft);
  const displayedMonthValues = draft.is_seasonal ? storedMonthValues : storedMonthValues.map(() => 1.0);
  const seasonalityScale = buildSeasonalityScale(displayedMonthValues);
  const statusLabel = saveInFlight ? "Salvando" : (savePending ? "Autosave pendente" : "Salvo");
  const buttonLabel = ["queued", "running"].includes(String(autofill.status || "")) ? "Buscando..." : "Gerar Dados (IA)";

  target.innerHTML = `
    <section class="truck-operations-section-box product-editor-operational-summary-box">
      <div class="product-editor-operational-toolbar">
        <span class="product-editor-operational-badge">${escapeHtml(statusLabel)}</span>
        <button
          class="editor-header-action product-editor-operational-autofill-button"
          type="button"
          data-operational-action="autofill"
          ${["queued", "running"].includes(String(autofill.status || "")) ? "disabled" : ""}
        >
          <span class="material-symbols-outlined">auto_awesome</span>
          <span>${escapeHtml(buttonLabel)}</span>
        </button>
      </div>
      ${autofill.message ? `<div class="product-editor-operational-status"><strong>${escapeHtml(autofill.message)}</strong>${autofill.error ? `<span>${escapeHtml(autofill.error)}</span>` : ""}</div>` : ""}
      ${autofill.summary ? `<p class="product-editor-operational-summary-text">${escapeHtml(autofill.summary)}</p>` : ""}
    </section>

    ${sectionBoxMarkup(
      "",
      [
        staticChoiceMarkup("Unidade", unit),
        inputChoiceMarkup("weight_per_unit_kg", "Peso por unidade (kg)", numericInputValue(draft.weight_per_unit_kg), { type: "number", min: 0, step: "0.001" }),
        inputChoiceMarkup("volume_per_unit_m3", "Volume por unidade (m3)", numericInputValue(draft.volume_per_unit_m3), { type: "number", min: 0, step: "0.0001" }),
      ].join(""),
    )}
    ...
    ${sectionBoxMarkup(
      "",
      [
        inputChoiceMarkup("price_reference_brl_per_unit", `Preco medio (R$ / ${unit})`, numericInputValue(draft.price_reference_brl_per_unit), { type: "number", min: 0, step: "0.01" }),
        inputChoiceMarkup("price_min_brl_per_unit", "Preco minimo", numericInputValue(draft.price_min_brl_per_unit), { type: "number", min: 0, step: "0.01" }),
        inputChoiceMarkup("price_max_brl_per_unit", "Preco maximo", numericInputValue(draft.price_max_brl_per_unit), { type: "number", min: 0, step: "0.01" }),
      ].join(""),
    )}
    ...
    ${sectionBoxMarkup(
      ...
      [
        `
          <label class="truck-operations-choice product-editor-operational-toggle-row">
            <span>Produto sazonal</span>
            <div class="product-editor-operational-toggle-control">
              <strong>${escapeHtml(draft.is_seasonal ? "Ativo" : "Desligado")}</strong>
              <input type="checkbox" data-operational-field="is_seasonal" ${draft.is_seasonal ? "checked" : ""} />
            </div>
          </label>
        `,
        `
          <article class="truck-operations-choice product-editor-seasonality-choice">
            <span>Grafico mensal</span>
            <div class="product-editor-seasonality-chart" aria-label="Grafico mensal de sazonalidade">
              ${SEASONALITY_MONTHS.map(({ label }, index) => {
                const value = displayedMonthValues[index];
                const ratio = Math.min(Math.abs(value - 1) / seasonalityScale.distance, 1);
                const height = value === 1 ? 2 : Math.max(6, Math.round(ratio * 50));
                const offset = value >= 1 ? 50 : 50 - height;
                const directionClass = value >= 1 ? "is-positive" : "is-negative";
                return `
                  <div class="product-editor-seasonality-bar-wrap">
                    <div class="product-editor-seasonality-bar-area" title="${escapeHtml(`Indice: ${formatNumber(value, 2)}`)}">
                      <div class="product-editor-seasonality-bar ${directionClass}" style="height:${height}%; bottom:${offset}%"></div>
                    </div>
                    <span>${escapeHtml(label)}</span>
                  </div>
                `;
...
async function saveOperationalRecord(productId = state.selectedProductId, { statusMessage = null } = {}) {
  const payload = buildOperationalSavePayload(productId);
  if (!payload) {
    return null;
  }

  clearOperationalAutosave(productId);
  const payloadSnapshot = serializeOperationalPayload(payload);
  state.operationalSavingByProductId[productId] = true;
  renderInformationPanel();

  state.operationalSaveChain = state.operationalSaveChain
    .catch(() => null)
    .then(async () => {
      const response = await fetch("/api/editor/products_v3/operational", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(data?.detail || "Falha ao salvar as informacoes do produto.");
      }

      rememberSavedOperationalRecord(data.operational_record, {
        overwriteDraft: serializeOperationalPayload(buildOperationalSavePayload(productId)) === payloadSnapshot,
      });
      renderInformationPanel();
      renderStatus(statusMessage || "Informacoes do produto salvas automaticamente.", [
        { label: "Produto", value: currentProduct()?.name || productId },
        { label: "Unidade", value: payload.unit || "-" },
      ]);
      return data;
    })
    .catch((error) => {
      renderStatus(error?.message || "Falha ao salvar as informacoes do produto.");
      throw error;
    })
    .finally(() => {
      delete state.operationalSavingByProductId[productId];
      renderInformationPanel();
    });

  return state.operationalSaveChain;
}

function scheduleOperationalAutosave(productId = state.selectedProductId) {
  if (!productId) {
    return;
  }
  clearOperationalAutosave(productId);
  state.operationalAutosaveTimersByProductId[productId] = window.setTimeout(() => {
    delete state.operationalAutosaveTimersByProductId[productId];
    void saveOperationalRecord(productId);
  }, OPERATIONAL_AUTOSAVE_DELAY_MS);
}

function stopOperationalAutofillPolling() {
  if (state.operationalAutofillPollTimer) {
    window.clearInterval(state.operationalAutofillPollTimer);
  }
  state.operationalAutofillPollTimer = null;
  state.operationalAutofillPollingProductId = "";
}

function startOperationalAutofillPolling(productId = state.selectedProductId) {
  if (!productId) {
    return;
  }
  stopOperationalAutofillPolling();
  state.operationalAutofillPollingProductId = productId;

  const poll = async () => {
    const query = new URLSearchParams({ product_id: productId });
    const response = await fetch(`/api/editor/products_v3/operational/autofill/status?${query.toString()}`);
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      throw new Error(payload?.detail || "Falha ao consultar o status do preenchimento automatico.");
    }
    state.operationalAutofillStatusByProductId[productId] = payload;
    if (payload.status === "completed") {
      rememberSavedOperationalRecord(payload.payload, { overwriteDraft: true });
      stopOperationalAutofillPolling();
      renderInformationPanel();
      renderStatus(payload.message || "Dados operacionais do produto gravados automaticamente.", [
        { label: "Produto", value: state.productsById[productId]?.name || productId },
      ]);
      return;
    }
    if (payload.status === "failed") {
      stopOperationalAutofillPolling();
      renderInformationPanel();
      renderStatus(payload.error || payload.message || "Falha ao gerar os dados do produto.");
      return;
    }
    renderInformationPanel();
  };
  ...
  void poll().catch((error) => {
    stopOperationalAutofillPolling();
    renderStatus(error?.message || "Falha ao consultar o status do preenchimento automatico.");
  });

  state.operationalAutofillPollTimer = window.setInterval(() => {
    void poll().catch((error) => {
      stopOperationalAutofillPolling();
      renderStatus(error?.message || "Falha ao consultar o status do preenchimento automatico.");
    });
  }, OPERATIONAL_AUTOFILL_POLL_INTERVAL_MS);
}
...
async function requestOperationalAutofill() {
  ...
  if (isOperationalDirty(product.id)) {
    await saveOperationalRecord(product.id, { statusMessage: "Informacoes do produto salvas antes do preenchimento automatico." });
  }
  const response = await fetch("/api/editor/products_v3/operational/autofill/background", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(payload?.detail || "Falha ao iniciar o preenchimento automatico do produto.");
  }
  state.operationalAutofillStatusByProductId[product.id] = payload;
  renderInformationPanel();
  startOperationalAutofillPolling(product.id);
}
...
function renderLegend(maxValue, product) {
  ...
  const summaryTarget = document.getElementById("product-editor-v1-map-summary");
  ...
  if (summaryTarget) {
    const supplyTotal = computeLayerTotal(product.id, "supply");
    const demandTotal = computeLayerTotal(product.id, "demand");
    summaryTarget.innerHTML = `
      <div class="product-editor-legend-card product-editor-map-summary-card">
        <div class="product-editor-map-summary-row">
          <span>Oferta Total:</span>
          <strong>${escapeHtml(formatUnitValue(supplyTotal, product.unit))}</strong>
        </div>
        <div class="product-editor-map-summary-row">
          <span>Demanda Total:</span>
          <strong>${escapeHtml(formatUnitValue(demandTotal, product.unit))}</strong>
        </div>
      </div>
    `;
  }
...
async function reloadBootstrap({ selectedProductId = null, selectedCityId = null, refitMap = true } = {}) {
  ...
  renderAll();
  const autofill = operationalAutofillStatus(state.selectedProductId);
  if (["queued", "running"].includes(String(autofill.status || ""))) {
    startOperationalAutofillPolling(state.selectedProductId);
  }
...
async function submitCreateProduct() {
  const draft = currentCreateDraft();
  const response = await fetch("/api/editor/products_v3/products", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
async function loadSelectedMap() {
  ...
  if (state.selectedMapId === state.bootstrap?.map_repository?.active_map_id) {
    renderStatus(messages().map_loaded || "Mapa de trabalho carregado no editor v3.");
    return;
  }
...
function bindControls() {
  ...
  document.getElementById("product-editor-v1-create-emoji")?.addEventListener("change", (event) => {
    currentCreateDraft().emoji = event.target.value || "📦";
  });
  ...
  document.getElementById("product-editor-v1-create-form")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      renderStatus("Produto incluido no catalogo do editor v3.");
    } catch (error) {
      renderStatus(error?.message || "Falha ao incluir o produto.");
    }
  });
  document.getElementById("product-editor-v3-information-panel")?.addEventListener("input", (event) => {
    const target = event.target;
    const field = target?.dataset?.operationalField;
    const product = currentProduct();
    const draft = currentOperationalDraft();
    if (!field || !product || !draft) {
      return;
    }
    ...
    if (target.type === "checkbox") {
      ...
      renderInformationPanel();
      scheduleOperationalAutosave(product.id);
      return;
    }

    if (field === "source_urls") {
      draft[field] = target.value || "";
    } else if (field === "research_basis" || field === "notes" || field === "confidence") {
      draft[field] = target.value || "";
    } else {
      draft[field] = toOptionalNumber(target.value);
    }
    scheduleOperationalAutosave(product.id);
  });
  document.getElementById("product-editor-v3-information-panel")?.addEventListener("change", (event) => {
    const target = event.target;
    const field = target?.dataset?.operationalField;
    const product = currentProduct();
    const draft = currentOperationalDraft();
    ...
    if (field === "source_urls") {
      draft[field] = normalizeSourceUrls(target.value || "");
  ...
  document.getElementById("product-editor-v3-information-panel")?.addEventListener("click", async (event) => {
    const button = event.target.closest("[data-operational-action='autofill']");
    if (!button) {
      return;
    }
    event.preventDefault();
    try {
      ...
      renderStatus("Pesquisa automatica de dados do produto iniciada.");
    } catch (error) {
      renderStatus(error?.message || "Falha ao iniciar a pesquisa automatica de dados do produto.");
    }
  });
...
initialize().catch((error) => {
  console.error("Brasix product editor v3 failed to initialize:", error);
  renderStatus(error?.message || "Falha ao iniciar o editor de produtos v3.");
});
