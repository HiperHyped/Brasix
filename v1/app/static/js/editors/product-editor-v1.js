...
function loadBootstrap() {
  return fetch("/api/editor/products_v1/bootstrap").then(async (response) => {
    if (!response.ok) {
      throw new Error(`Falha ao carregar bootstrap do editor de produtos v1 (${response.status}).`);
    }
...
function loadFieldDocument(productId, layer) {
  ...
  return fetch(`/api/editor/products_v1/field?${query.toString()}`).then(async (response) => {
    if (!response.ok) {
...
function saveFieldDocument(payload) {
  return fetch("/api/editor/products_v1/field", {
    method: "PUT",
...
function applyTheme(themeId, { persist = true } = {}) {
  ...
  applyCssVariables(state.bootstrap?.product_editor_v1?.layout_desktop?.css_variables || {});
...
function initializeState(payload) {
  ...
  state.screen = payload.product_editor_v1?.screen || {};
  state.shortcuts = payload.product_editor_v1?.shortcuts || { items: [] };
  state.themesDocument = payload.product_editor_v1?.themes || {};
...
function renderProductList() {
  ...
  const summary = document.getElementById("product-editor-v1-list-summary");
  if (!target) {
    return;
  }
  ...
  if (summary) {
    summary.textContent = `${visible.length} ${labels().list_count_label || "produtos visiveis"} - ${selectedStats.anchor_count || 0} ancoras - ${(currentDoc?.strokes || []).length} sessoes manuais`;
  }
  if (!visible.length) {
    target.innerHTML = "";
...
async function createProductFrom(sourceProductId = null) {
  ...
  const response = await fetch("/api/editor/products/products", {
    ...
    body: JSON.stringify({
      name: productName,
      emoji: baseProduct?.emoji || "\u{1F4E6}",
...
function bindControls() {
  ...
  document.getElementById("product-editor-v1-create-button")?.addEventListener("click", async () => {
    try {
  ...
  document.getElementById("product-editor-v1-duplicate-button")?.addEventListener("click", async () => {
    if (!currentProduct()) {
      return;
    }
    try {
